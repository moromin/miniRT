...
static t_color	calc_radiance_specular(t_object *obj, t_vector cross_point, t_light light, t_vector camera2cross);
...
t_color	calc_radiance(t_object *const me, t_vector cross_point, t_light light, t_vector camera2cross)
{
...
static t_color	calc_radiance_diffuse(t_object *obj, t_vector cross_point, t_light light, t_vector camera2cross)
{
	...
	const t_vector	normal = ({
		...
		if (obj->info.flag & (1 << FLAG_BUMPMAP))
			n = calc_bumpmap_normal(obj, cross_point);
		else
			n = calc_normal(obj, cross_point);
	...
	const t_color	col = calc_color(obj, cross_point);
