...
static double	object_intersect_at_(t_object *me, t_ray ray);
double			sphere_intersect_at(t_object *me, t_ray ray);

void	object_ctor(t_object *const me)
{
	static t_object_vtbl const	vtbl = {
			&object_intersect_at_
	};

	me->vptr = &vtbl;
}

static double	object_intersect_at_(t_object *const me, t_ray ray)
{
	(void)me;
	(void)ray;
	assert(0);
...
double	object_intersect_at(t_object *const me, t_ray ray)
{
	return ((*me->vptr->intersect_at)(me, ray));
}
 ...
 */
double	sphere_intersect_at(t_object *const me_, t_ray ray)
{
	const t_sphere	*me = (t_sphere *)me_;
	const double	abc[3] = {
			vec_magnitude_squared(ray.direction),
			2 * (vec_inner_product(ray.start, ray.direction)
				- vec_inner_product(ray.direction, me->center)),
			vec_magnitude_squared(ray.start)
				+ vec_magnitude_squared(me->center)
				- 2 * vec_inner_product(ray.start, me->center)
				- pow(me->radius, 2)
			};
